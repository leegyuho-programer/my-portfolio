  ...
  // MyPortfolio
  {
    ...
    mainWorks: [
      ...
      },
      {
        ...
        overview:
          '모달 표시 지연 문제를 해결하기 위해 IntersectionObserver와 Next.js의 `router.prefetch`를 조합해 프로젝트 상세 페이지 관련 리소스를 사전에 로드하는 전략을 도입했습니다.',
        contributions: [
          '`IntersectionObserver`를 활용해 **카드가 뷰포트 근처(100px 이내)에 들어올 때 상세 페이지 라우트를 자동 프리페칭**',
          '카드에 **마우스 오버/포커스 시점**에도 `router.prefetch`를 호출하여 사용자 행동 기반 사전 로딩 보완',
      ...
      {
        title: 'Jest + RTL 기반 단위/통합 테스트 적용',
        overview:
          '`Jest`와 `React Testing Library`를 도입해 단순한 동작 확인을 넘어 실제 **사용자 경험**과 가까운 **시나리오까지 검증**했습니다. 컴포넌트 단위의 안정성과 사용자 흐름 단위의 신뢰성을 동시에 확보하며 프로젝트 품질을 한 단계 끌어올렸습니다.',
        contributions: [
          '단위 테스트: 다양한 **UI 이벤트**와 **상태 변화**를 세밀하게 검증해 **리팩토링 이후에도 동일한 동작을 유지**할 수 있도록 품질 확보',
          '통합 테스트: **API 요청 → 데이터 반영 → 최종 UI 표시**의 흐름을 실제 시나리오로 검증해 **사용자 경험 신뢰성** 강화',
          '테스트 자동화를 **CI 파이프라인**에 포함시켜 **PR 단계 품질 확인**과 **안정적인 배포 사이클** 정착',
        ],
      },
      {
      ...
      },
      {
        ...
        resolutionMethod: {
          ...
          process: [
            {
              title:
                '**IntersectionObserver + 사용자 행동 기반 라우트 사전 로드 구현**',
              content: [
                {
                  description:
                    '`IntersectionObserver`를 활용하여 **카드가 뷰포트 100px 이내에 들어오면 해당 프로젝트 상세 경로를 `router.prefetch`로 사전 로드**하도록 구현',
                },
                {
                  description:
                    '추가적으로 사용자가 프로젝트 카드에 마우스 오버(`onMouseEnter`)하거나 포커스(`onFocus`)할 때도 `router.prefetch`를 호출해 **인터랙션 기반의 사전 로딩**을 보완',
                },
              ],
            },
          ],
        ...
        results: [
          ...
          '사전 로딩으로 인해 사용자 클릭 직후 네트워크 요청 지연 해소 → **모달 전환 반응성 개선**',
        ],
        learnings:
          "단순히 클릭을 기다리던 기존 방식이 사용자에게 얼마나 큰 지연 시간으로 다가갔는지 체감할 수 있었습니다. 이번에 `IntersectionObserver`를 활용해 **사용자의 시선이 머무를 만한 영역을 예측**하고 리소스를 미리 로드하는 전략을 적용하면서 성능 최적화에 대한 시각이 완전히 바뀌었습니다. 개발자에게는 보이지 않는 수십 ms의 개선이 사용자에게는 **'빠릿하다'와 '답답하다'의 차이**를 만든다는 것을 숫자로 확인했습니다. 이제는 단순히 기능을 구현하는 데 그치지 않고 **사용자의 다음 행동을 예측해 보이지 않는 곳에서 미리 준비해두는 습관**을 들여야겠다고 다짐하게 되었습니다.",
      },
    ],
  },
