    ...
    troubleShootings: [
      {
        ...
        background:
          'Article 페이지 전체 콘텐츠가 모두 준비된 후 렌더링되는 구조로 인해 주요 콘텐츠 표시까지 지연이 발생했고, 특히 댓글 영역의 비동기 로딩이 병목이 되어 LCP가 2.4초로 느린 편이었습니다.',
        resolutionMethod: {
          analysis: [
            '모든 컴포넌트가 준비될 때까지 HTML 전송이 지연됨',
            '비동기 컴포넌트가 초기 렌더링 병목을 유발',
            'React SSR이 페이지 전체를 묶어 렌더링하여 UX 저하',
          ],
          process: [
            'React Suspense 기반의 Streaming SSR을 도입해 주요 콘텐츠와 댓글을 <Suspense>로 분리',
            'fallback 컴포넌트를 사용하여 주요 콘텐츠 먼저 렌더링 후 댓글 병렬 로딩',
            '서버 로그를 활용해 렌더링 순서를 검증하여 Streaming SSR 정상 작동 확인',
          ],
        },
        results: [
          'LCP 2.4초 → 2.1초로 약 12.5% 개선',
          '중요 콘텐츠가 더 빠르게 렌더링되어 사용자 체감 속도 향상',
          '댓글 컴포넌트를 백그라운드 로딩하여 초기 UX 분리',
        ],
        learnings:
          'Streaming SSR을 통해 주요 콘텐츠를 우선 렌더링하면 초기 반응성과 SEO를 동시에 개선할 수 있으며, React Suspense와 Next.js App Router의 조합이 유용하다는 점을 체감했습니다.',
      },
      {
        title: 'SVG 최적화 및 렌더링 성능 개선',
        background:
          'SVG 아이콘을 인라인 삽입하거나 <Image>로 불러올 경우, 파일 용량 증가와 중복 속성으로 인해 렌더링 속도가 저하되고 LCP 점수가 낮아졌습니다.',
        resolutionMethod: {
          analysis: [
            'SVG에 불필요한 속성과 중복된 코드 포함됨',
            '인라인 SVG로 인해 DOM 복잡도 증가',
            '스타일 오버라이드 어려움 및 SVG 파일 크기 증가로 렌더링 성능 저하',
          ],
          process: [
            'SVGR로 SVG를 React 컴포넌트화하여 props 기반 스타일 변경 지원',
            'SVGO를 사용해 속성 정리 및 용량 최적화',
            '자주 쓰는 아이콘은 컴포넌트, 단발성 이미지는 next/image로 처리',
          ],
        },
        results: [
          'LCP 최대 콘텐츠 페인트 약 28% 단축',
          '렌더링 속도 44% 향상',
          '최대 지연 시간 51% 감소',
        ],
        learnings:
          'SVG 최적화는 렌더링 성능과 유지보수 모두에 긍정적인 영향을 주며, SVGR과 SVGO 조합은 아이콘 관리에 효율적인 방법임을 알게 되었습니다.',
      },
      {
        title: 'Next.js 내장 모달 라우팅 vs React 포탈 선택',
        background:
          'Next.js의 모달 라우팅을 사용했지만, URL 변경과 상태 관리의 복잡성이 커지면서 UI 유지 관리가 어려워졌습니다.',
        resolutionMethod: {
          analysis: [
            'URL 상태와 모달 UI 상태를 일관되게 동기화해야 하며, 이로 인해 코드 복잡도 상승',
            '라우팅 변화에 따른 예외 처리 및 side effect 관리가 필요',
          ],
          process: [
            '초기에는 route intercepting 기반 모달 구현',
            '라우팅 복잡도와 상태 충돌을 경험한 후, React Portal 기반 모달로 전환',
            '전역 layout에 모달 portal 위치를 지정하고 상태는 context로 관리',
          ],
        },
        results: [
          'URL 상태와 분리된 모달 제어로 유지보수 간소화',
          '브라우저 히스토리 간섭 없이 모달 상태를 독립적으로 관리 가능',
        ],
        learnings:
          '모달 상태 관리 복잡도에 따라 라우팅 기반 방식보다 React Portal이 더 적절할 수 있으며, 기술 선택은 요구되는 UX 및 관리 비용에 따라 달라져야 함을 깨달았습니다.',
      },
    ],
  },
  {
    ...
    mainWorks: [
      {
        ...
        overview:
          '복잡한 입력 필드 상태 관리를 단순화하고 성능을 개선했습니다.',
        contributions: [
          '`react-hook-form`의 `Controller`를 사용하여 폼 상태와 유효성 검사 로직 통합',
          '불필요한 리렌더링 제거 및 코드 재사용성 향상',
        ],
      },
      {
        title: '이미지 최적화를 통한 페이지 로드 성능 개선',
        overview: '고화질 이미지로 인한 페이지 성능 저하를 개선했습니다.',
        contributions: [
          'AVIF 포맷 도입 및 Next.js `Image` 컴포넌트를 활용한 lazy loading 적용',
          '이미지 품질 유지와 함께 로딩 시간 최적화',
        ],
      },
      {
        title: 'react-query로 API 통신 최적화',
        overview:
          'API 요청 최적화를 통해 성능 향상 및 데이터 동기화를 효율화했습니다.',
        contributions: [
      ...
      },
      {
        ...
        overview:
          '계정 관련 핵심 페이지(Mypage, 프로필 수정 등)를 일관된 UI/UX로 구축했습니다.',
        contributions: [
      ...
      },
      {
        ...
        overview:
          '모바일, 태블릿, 데스크탑에서 최적의 사용자 경험을 제공하기 위한 UI 대응 작업',
        contributions: [
          'Tailwind CSS로 반응형 레이아웃 구현',
          '접근성과 편의성을 고려한 디바이스 호환성 확보',
        ],
      },
    ],
    troubleShootings: [
      {
        ...
        background:
          'useState로 개별 관리하던 폼 상태와 유효성 로직이 중복되며 유지보수성이 저하되고, 리렌더링 성능에도 문제가 있었습니다.',
        resolutionMethod: {
          analysis: [
            'useState 기반 관리로 인해 상태 코드가 장황하고 이벤트 핸들러가 복잡',
            '중복 로직과 리렌더링 발생',
          ],
          process: [
            '`react-hook-form`과 `Controller`를 도입하여 상태 및 유효성 검사 통합',
            '중복 코드 제거 및 불필요한 렌더 제거로 성능 최적화',
          ],
        },
        results: [
        ...
        learnings:
          '폼 상태 관리를 효과적으로 설계하면 성능 및 유지보수 모두에 긍정적이며, `react-hook-form`은 폼 유효성 검사에 매우 적합한 도구임을 알게 됐습니다.',
      },
      {
        title: '팀 간 소통 미흡으로 인한 작업 병목 발생',
        background:
          '프론트/백엔드, 디자이너 간 커뮤니케이션이 원활하지 않아 요구사항 전달과 개발 타임라인에 문제가 발생했습니다.',
        resolutionMethod: {
          analysis: [
            '비동기 피드백으로 인한 결정 지연',
            '우선순위 불일치로 인해 병목 발생',
          ...
          process: [
            '평일 19~22시 코어타임 지정으로 실시간 협업',
            'GitHub Discussions를 활용한 데일리 스크럼 도입 (Block / Done / To-do 공유)',
          ],
        },
        results: [
          '팀원 간 작업 진행 상황 실시간 공유 및 대응 속도 향상',
          '프로젝트 일정에 맞춘 효율적인 협업 체계 확립',
        ],
        learnings:
          '코어타임과 데일리 스크럼을 통한 정기적인 소통이 협업 효율을 높이고, 프로젝트 퀄리티에 직접적으로 기여함을 체감했습니다.',
      },
    ],
  },
  {
