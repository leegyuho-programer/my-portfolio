    ...
    mainWorks: [
      {
        ...
        contributions: [
          ...
          '모달 URL 접근 시에도 동일한 **모달 UI 유지**로 사용자 경험 최적화',
        ],
      },
      {
        title: '라우트 프리페칭 기반 프로젝트 상세 모달 로딩 최적화',
        overview:
          '모달 표시 지연 문제를 해결하기 위해 Next.js의 `router.prefetch`를 활용하여 프로젝트 상세 페이지 관련 리소스를 사전에 로드하는 전략을 도입했습니다. 이를 통해 사용자 경험을 개선하고 애플리케이션의 전반적인 반응 속도를 최적화했습니다.',
        contributions: [
          '카드에 **마우스 오버/포커스** 시 해당 상세 페이지 라우트를 미리 로드하도록 구현',
          '컴포넌트 마운트 시 필터링된 모든 프로젝트에 대해 백그라운드 프리페칭을 적용하여 클릭 시 로딩 시간 최소화',
          '모달 렌더링 관련 스크립트 실행 시간을 **약 41% 감소**시키고, 모달을 포함한 페이지의 전체 이벤트 시간을 **약 30% 단축**하여 반응 속도 향상 달성',
        ],
      },
      {
        title: '상태 기반 모달 구현을 통한 기술 스택 설명 기능 개발',
        overview:
          '단순 UI 오버레이의 경우 Intercepting Routes보다 상태 기반 모달이 적합하다는 판단 하에 TechModal에 적용했습니다.',
        contributions: [
          'React의 `useState`훅을 활용한 **상태 기반 조건부 렌더링** 방식으로 TechModal 구현',
          '**URL과 분리된 상태 관리**로 코드 복잡도 및 예측 불가능한 라우팅 이슈 방지',
          '**브라우저 히스토리 오염 없이** 안정적인 사용자 경험 제공',
        ],
      },
      {
      ...
      },
      {
        ...
        overview:
          'Next.js 앱에서 이미지 지연 로딩 및 깜빡임 현상을 해결하고, Vercel CDN 캐시 활용을 통해 **LCP 48% 단축**, **이미지 로딩 시간 약 86.7% 단축** 등의 성능 최적화를 달성했습니다.',
        contributions: [
          '`public` 폴더 내 이미지 리소스를 CDN 효율적으로 서빙하도록 `vercel.json`을 설정하여 캐시 정책(Cache-Control: public, max-age=31536000, immutable) 명시',
          '불필요한 re-render 방지 및 모달 전후 이미지 재로딩 이슈 해결을 위해 컴포넌트 수준에서 렌더링 전략 개선',
          '최적화 적용 후, 이미지 로딩 시간 약 **450ms → 59.73ms**로 단축, **CDN 캐시 HIT율 10~20% → 90%+**, **LCP 2.3s → 1.2s**로 성능 지표를 객관적으로 개선',
        ],
      },
    ],
    ...
    troubleShootings: [
      {
        ...
        background:
          '프로젝트의 ProjectsSection에서 카드 클릭 시 모달이 열리고 닫히는 구조였는데, 모달 종료 후 카드 이미지가 다시 렌더링되면서 **이미지 깜빡임**과 **지연 로딩 현상**이 발생했습니다. 이는 사용자 경험을 저해하고, 퍼포먼스 이슈로도 이어졌습니다. 이미지 파일은 모두 `public/Images`에 정적으로 저장되어 있었습니다.',
        resolutionMethod: {
          analysis: [
        ...
        results: [
          ...
          'LCP(Largest Contentful Paint) **2.3초 → 1.2초**로 약 48% 성능 개선',
      ...
      },
      {
        ...
        results: [
          '**모달을 포함한 전체 이벤트 총 시간:** **5,478ms → 3,844ms**로 **약 1.63초(29.7%) 단축**',
          '**모달 렌더링 관련 스크립트 실행 시간:** **237.7ms → 138.3ms**로 **약 41.8% 감소**',
          '**스크립트 컴파일 시간:** **253ms → 148ms**로 **약 41.4% 감소**',
          '**스크립트 평가 시간:** **233.5ms → 126.3ms**로 **약 45.9% 감소**',
