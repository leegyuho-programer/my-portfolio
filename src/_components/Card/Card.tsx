...
export default function Card({ children, projectId }: CardProps) {
  return (
    <div className='group relative rounded-lg w-[350px] h-[410px] bg-lightGray overflow-hidden hover:shadow-black hover:shadow-2xl hover:translate-y-[-5px] hover:bg-[#323232]'>
      {children}
    </div>
  );
}
...
Card.Hover = function CardHover({
  ...
  return (
    <div
      className={`absolute inset-0 ${flexColCenter} transition-opacity gap-10 duration-300 opacity-0 group-hover:opacity-100 z-10 bg-[#323232] bg-opacity-90`}
    >
      <h3 className='text-xl font-bold text-white'>{title}</h3>
      <Link
        href={`/project/${projectId}`}
        className='py-3 px-8 border-[1px] border-solid border-white text-sm text-white rounded-lg transition hover:bg-white hover:text-black'
      >
        {text}
      </Link>
    </div>
